...
const createBooking = async (req, res) => {
  try {
    const {
      houseId, houseName, houseType, district, sector, cell, village,
    ...
    if (!fullName || !email || !phone || !houseId || !houseName || !checkIn || !checkOut || !months || !guests) {
    ...
    const booking = new Booking({
      ...
      months: parseInt(months), guests: parseInt(guests),
      specialRequests,
    ...
    const emailData = {
      ...
      fullName, email, phone, houseName, district, sector,
      checkIn: booking.checkIn, checkOut: booking.checkOut,
      months: booking.months, guests: booking.guests,
      specialRequests,
      monthlyRent: booking.monthlyRent,
      serviceFee: booking.serviceFee,
      totalAmount: booking.totalAmount,
      ownerName, ownerContact, university,
    ...
    if (ownerEmail) {
      ...
      await sendEmail(ownerEmail, ownerEmailTemplate.subject, ownerEmailTemplate.html);
    } else {
      const ownerEmailTemplate = ownerNotificationEmail(emailData);
      await sendEmail(process.env.SMTP_USER, ownerEmailTemplate.subject, ownerEmailTemplate.html);
    }
...
const getBookingStats = async (req, res) => {
  try {
    ...
    const paymentPending = await Booking.countDocuments({ paymentStatus: 'pending' });
    const paymentVerified = await Booking.countDocuments({ paymentStatus: 'verified' });
    const paymentFailed = await Booking.countDocuments({ paymentStatus: 'failed' });

    const revenueResult = await Booking.aggregate([
      { $match: { paymentStatus: 'verified', status: { $in: ['confirmed', 'completed'] } } },
    ...
    const monthlyBookings = await Booking.aggregate([
      {
        $group: {
          ...
          count: { $sum: 1 },
          revenue: { $sum: { $cond: [{ $eq: ['$paymentStatus', 'verified'] }, '$totalAmount', 0] } }
        }
      },
    ...
    res.json({
      ...
      data: {
        ...
        byPayment: { pending: paymentPending, verified: paymentVerified, failed: paymentFailed },
        totalRevenue,
        monthlyBookings
      }
    });
