...
const sendAdminRequestEmail = async (request) => {
  try {
    const adminEmail = "inyumbarental@gmail.com";
    const adminTemplate = getAdminRequestNotificationEmail(request);
...
const sendRequestEmails = async (request, type, userInfo = null) => {
  try {
    ...
    if (type === "request_created") {
      ...
      // Send admin email
      const adminResult = await sendAdminRequestEmail(request);
      if (adminResult.success) {
        emailsSent.push({ to: adminResult.adminEmail, role: "admin" });
      }
    }

    // Send notification to admin (for updates)
    if (type === "request_updated") {
      ...
      // Also send to fixed admin email
      const fixedAdminResult = await sendAdminRequestEmail(request);
      if (fixedAdminResult.success) {
        emailsSent.push({ to: fixedAdminResult.adminEmail, role: "admin" });
      }
    }
